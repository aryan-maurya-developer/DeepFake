#!/usr/bin/env python3
"""
DeepFake Advanced Meta-Learner Training Suite (train_meta_learner_advanced.py)
==============================================================================
...
7.  Artifact Generation:
    ...
    - The final, best-performing trainable meta-learner and its associated
      ...
      subfolders (e.g., api_artifacts_dir/image/deepfake_meta_learner.joblib).
...
----------
  ...
  --api-artifacts-dir API_ARTIFACTS_DIR
                        ...
                        (e.g., ./api/meta_model_artifacts/image/deepfake_meta_learner.joblib).
...
python train_meta_learner_advanced.py \\
    ...
    --api-artifacts-dir ./deepfake_private/api/meta_model_artifacts \\
    --optimizer optuna \\
    --optuna-trials 100
...
API-ready artifacts like `deepfake_meta_learner.joblib` into
`./deepfake_private/api/meta_model_artifacts/image/`.
"""
...
def run_meta_learning_experiments(
    ...
    console.rule(
        f"[bold cyan]DeepFake Meta-Learning: {media_type.upper()} (Optimizer: {OPTIMIZER_CHOICE})[/bold cyan]"
    )
    console.print(
        Panel(
    ...
    joblib.dump(
        ...
        os.path.join(media_type_api_artifacts_subdir, "deepfake_meta_imputer.joblib"),
    )
    joblib.dump(
        ml_preprocessor.named_steps["scaler"],
        os.path.join(media_type_api_artifacts_subdir, "deepfake_meta_scaler.joblib"),
    )

    api_feature_cols_path = os.path.join(
        media_type_api_artifacts_subdir, "deepfake_meta_feature_columns.json"
    )
    ...
    if best_trainable_ml_model_for_api:
        api_model_joblib_path = os.path.join(
            media_type_api_artifacts_subdir, "deepfake_meta_learner.joblib"
        )
        ...
        console.print(
            f"[yellow]To deploy a trainable ML model, choose the best one from this run and ensure its '.joblib' is saved as 'deepfake_meta_learner.joblib' inside '{media_type_api_artifacts_subdir}'.[/yellow]"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Run Meta-Learning Experiments for DeepFake Ensemble."
    )
    parser.add_argument(
        "--media-type",
        type=str,
