...
def create_app(manifest: ModelManifest, model_dir: str) -> FastAPI:
    ...
    instance: DeepFakeModel = model_cls(name=manifest.name, model_dir=model_dir)
    ...
    app = FastAPI(
        title=f"DeepFake Model: {manifest.name}",
...
def cli():
    ...
    parser = argparse.ArgumentParser(prog="deepfake")
    ...
    if args.command == "serve":
        ...
        logger.info(f"Starting DeepFake model server: {manifest.name}")
