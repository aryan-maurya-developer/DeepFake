...
from deepfake_sdk.manifest import WeightEntry
from deepfake_sdk.weights import ensure_weights, compute_sha256
...
def test_ensure_weights_skips_existing(tmp_path):
    ...
    with patch("deepfake_sdk.weights.urllib.request.urlretrieve") as mock_dl:
...
def test_ensure_weights_downloads_missing(tmp_path):
    ...
    with patch(
        "deepfake_sdk.weights.urllib.request.urlretrieve", side_effect=fake_download
    ):
        ensure_weights([entry], str(tmp_path))
    filepath = os.path.join(str(tmp_path), "model.pth")
