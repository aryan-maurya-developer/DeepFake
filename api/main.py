#!/usr/bin/env python3
"""
DeepFake API Gateway
====================

The central orchestration layer for the DeepFake platform. This service acts as a unified gateway
...
Configuration:
Driven by `deepfake_config.json`, allowing for dynamic registration of new model endpoints without code changes.
"""
...
_default_config_path = os.path.join(os.path.dirname(_api_dir), "config", "deepfake_config.json")
CONFIG_FILE_PATH_FROM_ENV = get_environment_variable("DEEPFAKE_CONFIG_FILE_PATH", default=_default_config_path)
...
SECRET_KEY = get_environment_variable(
    "SECRET_KEY", "deepfake_super_secret_key_change_me"
)
ALGORITHM = "HS256"
    ...
    for m_type in SUPPORTED_MEDIA_TYPES:
        ...
        try:
            ...
            model_path = os.path.join(
                media_type_artifact_subdir, "deepfake_meta_learner.joblib"
            )
            scaler_path = os.path.join(
                media_type_artifact_subdir, "deepfake_meta_scaler.joblib"
            )
            imputer_path = os.path.join(
                media_type_artifact_subdir, "deepfake_meta_imputer.joblib"
            )
            cols_path = os.path.join(
                media_type_artifact_subdir, "deepfake_meta_feature_columns.json"
            )
...
async def root_api():
    ...
    return {
        "name": "DeepFake API",
        "version": app.version,
        "message": "Welcome to the DeepFake multimodal deepfake detection API.",
...
def print_results_summary_table_api(
    ...
    table = RichTable(
        title=f"DeepFake API Analysis (Req ID: {request_id}, Media: {media_type.upper()})",
        show_lines=True,
    )
...
if __name__ == "__main__":
    if (
        ...
        logger.critical(
            "FATAL: API configuration (DEEPFAKE_CONFIG_FILE_PATH) is missing, invalid, or does not define 'media_types'. API cannot start meaningfully."
        )
        sys.exit(1)
    ...
    logger.info(
        f"Starting DeepFake API (v{app.version}) on port {port} with {workers} worker(s). Log level: {log_level}"
    )
