...
export const exportResults = (result, fileName) => {
  ...
  const exportFileDefaultName = `deepfake_analysis_${fileName}_${new Date().toISOString().slice(0,10)}.json`;
...
export const downloadPDF = (result, fileName, preview) => {
  ...
  pdf.text('Generated by DeepFake - Advanced AI Deepfake Detection', 20, 280);
  
  // Save the PDF
  pdf.save(`deepfake_report_${fileName}_${new Date().toISOString().slice(0,10)}.pdf`);
};