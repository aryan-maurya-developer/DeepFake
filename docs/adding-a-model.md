# Adding a New Model to DeepFake

DeepFake uses a shared SDK so you only write the model-specific logic. Everything else — the HTTP server, health checks, lazy loading, thread safety — is handled for you.
...
```python
import numpy as np
from deepfake_sdk import VideoModel, PredictionResult
...
```python
from deepfake_sdk import AudioModel, PredictionResult
...
EXPOSE 5020
CMD ["deepfake", "serve", "--manifest", "model.yaml"]
```
...
Add to config/deepfake_config.json under the matching media type:
...
Your model is now part of the DeepFake ensemble.
