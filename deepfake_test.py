#!/usr/bin/env python3
"""
DeepFake Unified Testing Tool & Experimentation Suite (deepfake_test.py)
========================================================================

This script serves as a comprehensive command-line interface for testing,
evaluating, and managing the DeepFake deepfake detection system. It interacts
with the main DeepFake API and its individual model microservices for various
...
Core Usage Pattern:
-------------------
./deepfake_test.py [COMMAND] --media-type [image|video|audio] [OPTIONS]
...
1. Check System Health (for image models):
   ./deepfake_test.py health --media-type image

2. Test a Single Image with the Main API (using default "stacking" ensemble):
   ./deepfake_test.py test --media-type image --input /path/to/image.jpg

3. Test a Single Image with a Specific Individual Model:
   ./deepfake_test.py test --media-type image --input /path/to/image.jpg --model npr_deepfakedetection

4. Test a Single Video with the Main API (using 'average' ensemble):
   ./deepfake_test.py test --media-type video --input /path/to/video.mp4 --method average
...
5. Process a Batch of Images (evaluating all configured image models and local voting ensemble):
   ...
   ./deepfake_test.py batch --media-type image --input-dir ./dataset/images_real_fake/

6. Process a Batch of Audio files with a Specific Audio Model:
   ./deepfake_test.py batch --media-type audio --input-dir ./dataset/audio_clips/ --model example_audio_model_1

7. Compare Model Performance for Videos:
   ./deepfake_test.py compare --media-type video --input-dir ./dataset/eval_videos/ --output-dir ./reports/video_comparison/

8. Benchmark API and Image Model Latency:
   ./deepfake_test.py benchmark --media-type image --input /path/to/sample_image.jpg --count 15

9. Test the Deployed API's Stacking Ensemble on a Batch of Images:
   ./deepfake_test.py apistacktest --media-type image --input-dir ./dataset/eval_images/

10. Generate Visualizations from a Previous Batch Run's Results JSON:
    ./deepfake_test.py visualize --input-path ./deepfake_test_results/YYYYMMDD_HHMMSS/batch_results_image_my_dataset.json --output-dir ./viz_outputs/
...
11. Process a Directory of Videos and then Generate All Visualizations:
    ./deepfake_test.py visualize --media-type video --input-path ./dataset/test_videos/ --plot-type all
...
Notes:
------
- Ensure all DeepFake services (API gateway, model microservices for the
...
# Custom utilities from deepfake_utils package
from deepfake_utils.config_manager import ConfigManager
from deepfake_utils.api_client import APIClient
from deepfake_utils.media_handler import MediaHandler
from deepfake_utils.results_processor import ResultsProcessor
from deepfake_utils.visualizer import Visualizer
...
class DeepFakeTestOrchestrator:
    def __init__(
        ...
        _output_dir_base = cli_output_dir_base or self.config_manager.get_default(
            "default_output_dir_base", "./deepfake_test_results"
        )
        ...
        try:
            ...
            self.current_run_output_dir = f"./deepfake_temp_results_{timestamp}"
            os.makedirs(self.current_run_output_dir, exist_ok=True)
            console.print(
    ...
    def execute_health_check(self):
        ...
        console.print(
            Panel(
                f"[bold cyan]DeepFake System Health Check (Media Type: {self.media_type.upper()})[/bold cyan]",
                border_style="blue",
                expand=False,
            )
        )
        ...
        api_table = Table(title="DeepFake System Status")
    ...
    def execute_single_test(
        ...
        console.print(
            Panel(
                f"[bold cyan]DeepFake Single {self.media_type.capitalize()} Test[/bold cyan]\n"
    ...
    def execute_batch_processing(
        ...
        console.print(
            Panel(
                f"[bold cyan]DeepFake Batch Processing ({self.media_type.capitalize()})[/bold cyan]\n"
    ...
    def execute_comparison(
        ...
        console.print(
            Panel(
                f"[bold cyan]DeepFake Model Comparison ({self.media_type.capitalize()})[/bold cyan]\n"
                f"Input Dir: {input_dir}\nModel(s): {specific_model_name or 'All for ' + self.media_type}\n"
                f"Threshold: {threshold:.2f}\nLocal Ensemble: {local_ensemble_method.capitalize()}\n"
    ...
    def execute_api_stacking_test(self, input_dir: str, threshold: float):
        ...
        console.print(
            Panel(
                f"[bold cyan]DeepFake API Stacking Ensemble Test ({self.media_type.capitalize()})[/bold cyan]\n"
    ...
    def execute_benchmark(self, media_path: str, count: int):
        ...
        console.print(
            Panel(
                f"[bold cyan]DeepFake Performance Benchmark ({self.media_type.capitalize()})[/bold cyan]\n"
                f"Media File: {media_path}\nIterations: {count}\n"
                f"Output Dir: {os.path.abspath(self.current_run_output_dir)}",
    ...
    def execute_visualization(self, input_path: str, plot_type: str):
        ...
        console.print(
            Panel(
                f"[bold cyan]DeepFake Visualization Generation[/bold cyan]\n"
...
def main_cli():
    parser = argparse.ArgumentParser(
        description="DeepFake Unified Testing Tool & Experimentation Suite.",
    ...
    orchestrator = DeepFakeTestOrchestrator(
...
if __name__ == "__main__":
    if not config_manager.is_config_loaded_successfully():
        console.print(
            "[bold red]Exiting due to configuration load failure. Please check `config/deepfake_config.json`.[/bold red]"
        )
        sys.exit(1)
    try:
        main_cli()
