#!/usr/bin/env python3
"""
CLI tool to register a new detection model into the DeepFake platform.

Automates:
  1. Adding model/health endpoints to config/deepfake_config.json
...
CONFIG_PATH = os.path.join(PROJECT_ROOT, "config", "deepfake_config.json")
...
def update_config(name, media_type, port):
    """Add model and health endpoints to deepfake_config.json."""
...
def update_compose(name, media_type, port):
    ...
    container_name = f"deepfake-{name.replace('_', '-')}"
...
def main():
    parser = argparse.ArgumentParser(
        description="Add a new model to the DeepFake platform"
    )
